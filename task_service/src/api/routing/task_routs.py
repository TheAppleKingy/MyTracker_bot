...
from api.dependencies import get_task_service, get_task_repo, check_permissions
...
async def create_task(task_data: TaskCreateSchema, request_user: User = Depends(check_permissions(GroupPermission(['Admin']), IsActivePermission())), task_repo: TaskRepository = Depends(get_task_repo)):
...
async def update_task(id: int, data_to_update: TaskUpdateSchema, request_user: User = Depends(check_permissions(GroupPermission(['Admin']), IsActivePermission())), task_repo: TaskRepository = Depends(get_task_repo)):
