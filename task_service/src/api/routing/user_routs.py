...
from api.dependencies import check_permissions, get_task_service, get_user_repo
...
async def create_user(user_data: UserCreateSchema, request_user: User = Depends(check_permissions(GroupPermission(['Admin']), IsActivePermission())), user_repo: UserRepository = Depends(get_user_repo)):
...
async def update_user(id: int, data_to_update: UserUpdateSchema, request_user: User = Depends(check_permissions(GroupPermission(['Admin']), IsActivePermission())), user_repo: UserRepository = Depends(get_user_repo)):
