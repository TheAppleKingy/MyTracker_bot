...
from api.dependencies import get_group_service, check_permissions
...
async def add_user_in_group(id: int, data: GroupUpdateSchema, request_user: User = Depends(check_permissions(GroupPermission(['Admin']), IsActivePermission())), group_service: GroupService = Depends(get_group_service)):
    return await group_service.add_users(id, data.users)


@group_router.patch('/{id}/exclude_users', response_model=list[UserViewSchema])
async def exclude_users_from_group(id: int, data: GroupUpdateSchema, request_user: User = Depends(check_permissions(GroupPermission(['Admin']), IsActivePermission())), group_service: GroupService = Depends(get_group_service)):
