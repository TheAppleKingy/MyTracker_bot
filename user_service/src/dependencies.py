from fastapi import Depends, Cookie

from sqlalchemy.ext.asyncio import AsyncSession

from typing import Literal

from database import get_db_session
from repository.socket import DBSocket, SocketFactory, T
from service.factories import UserServiceFactory, GroupServiceFactory
from service.user_service import UserService
from service.group_service import GroupService
from models.models import User, Group


def get_socket(model: T):
    async def socket(session: AsyncSession = Depends(get_db_session)):
        return SocketFactory.get_socket(model, session)
    return socket


def get_user_service(socket: DBSocket = Depends(get_socket(User))):
    service = UserServiceFactory.get_service(socket)
    return service


def get_group_service(socket: DBSocket = Depends(get_socket(Group))):
    service = GroupServiceFactory.get_service(socket)
    return service


# async def authenticate(access: str = Cookie(default=None, include_in_schema=False), socket: DBSocket = Depends(get_socket(User))):
#     """dependency for getting authenticated by token user obj"""
#     from security.authentication import JWTAuther
#     user_service = UserServiceFactory.get_service(socket)
#     return await JWTAuther(user_service).auth(access)


# async def authenticate_by_refresh(refresh: str = Cookie(default=None, include_in_schema=False), socket: DBSocket = Depends(get_socket(User))):
#     from security.authentication import JWTAuther
#     user_service = UserServiceFactory.get_service(socket)
#     return await JWTAuther(socket).auth(refresh, 'refresh')


def jwt_authentication(type: Literal['refresh', 'access'] = 'access'):
    async def authenticate(access: str = Cookie(default=None, include_in_schema=False), refresh: str = Cookie(default=None, include_in_schema=False), user_service: UserService = Depends(get_user_service)):
        from security.authentication import JWTAuther
        token_map = {
            'access': access,
            'refresh': refresh
        }
        return await JWTAuther(user_service).auth(token_map[type], type)
    return authenticate


def get_user_allowed_by_group(allowed_group: str):
    async def check_user(user: User = Depends(jwt_authentication()), group_service: GroupService = Depends(get_group_service)):
        from permissions import GroupPermission
        return await GroupPermission(allowed_group, group_service).check(user)
    return check_user
