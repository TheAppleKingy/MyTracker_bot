...
class CeleryNotifyService(NotifyServiceInterface):
    def send_notify(self, tg_name: str, message_text: str, chat_id: int, scheduled_time: datetime) -> str:
        async_res = notify.apply_async(args=[message_text, chat_id, tg_name], eta=scheduled_time)
        return async_res.id
