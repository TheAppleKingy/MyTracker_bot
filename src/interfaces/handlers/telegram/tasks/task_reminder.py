...
from .shared import SetReminder
...
#     id_ = notify_service.send_notify(event.from_user.username,
#                                      data["task_id"], msg, event.message.chat.id, remind_datetime_utc)
#     await storage.set_reminder(event.from_user.username, remind_datetime_utc, id_, data["task_id"])
#     await event.message.edit_text(
...
#     delta_str = show_timedelta_verbose(delta)
#     msg = f'<b>Task "{data["task_title"]}" is waiting! Deadline over ' + delta_str + "</b>"
#     id_ = notify_service.send_notify(event.from_user.username, data["task_id"], msg, event.chat.id, remind_datetime_utc)
#     await storage.set_reminder(event.from_user.username, remind_datetime_utc, id_, data["task_id"])
...
async def set_remind_hour(
    event: types.CallbackQuery,
    context: FSMContext,
    use_case: FromDishka[SetReminder]
):
    await event.answer()
    suf = event.data.split("_")[-1]
    if suf == "manually":
...
async def show_reminders(
    ...
    reminders_tab = await storage.get_reminders_tab(event.from_user.username, task_id)
...
async def show_reminder_actions(
    ...
    task_id, reminder_id = int(data[0]), data[1]
    reminder: datetime = await storage.get_reminder(event.from_user.username, reminder_id, task_id)
    user_tz = await storage.get_tz(event.from_user.username)
    return await event.message.answer(
        text=f"<b>{reminder.astimezone(user_tz).strftime("%d.%m.%Y at %H:%M")}</b>",
        reply_markup=under_reminder_kb(reminder_id, task_id),
        parse_mode="HTML"
    )
...
async def delete_reminder(
    ...
    data = event.data.split("_")[2:]
    task_id, reminder_id = int(data[0]), data[1]
    await storage.delete_reminders(event.from_user.username, [reminder_id], task_id)
